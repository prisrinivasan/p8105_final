...
## _Linear Model of Cockroach Presence, Poverty, and Neighobrhood on Asthma_
```{r, echo = FALSE}
...
linear_model = 
  lm(public_school_children_5_14_yrs_old_with_asthma ~ homes_with_cockroaches * poverty * time_period, data = ggplot_data) %>%
  broom::tidy()

ggplot(ggplot_data, aes(x = homes_with_cockroaches, y = public_school_children_5_14_yrs_old_with_asthma, color = poverty)) +
  geom_point() + 
  geom_smooth(method = "lm") +
  facet_grid(~time_period) +
  labs(x = "Percent of Homes with Cockroaches",
       y = "Percent of School Children with Asthma",
       title = "Effect of Homes with Cockroaches on Childhood Asthma adjusting for Poverty") +
  viridis::scale_color_viridis(aes(color = poverty))

```

## _Table of the Values in Linear Model_
```{r, echo = FALSE}
